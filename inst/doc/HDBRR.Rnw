...
%% almost as usual
\author{P\'erez-Elizalde S. \\Colegio de Postgraduados$^{*}$ \And \,\,
        Monroy-Castillo B.$^{*}$ \And \,\,\,\,\,\,\,\,\quad
...
\Plainauthor{Sergio P\'erez-Elizalde, Blanca Monroy-Castillo, Paulino P\'erez-Rodr\'iguez} %% comma-separated
\Plaintitle{HDBRR: A Statistical Package for a High Dimension Ridge Regression withouth MCMC} %% without formatting
\Shorttitle{\pkg{HDBRR}: An R-package for Ridge Regression withouth MCMC} %% a short title (if necessary)
...
Ridge regression is a useful tool to deal with collinerity in
 ...
 independent. However, from both classical and Bayesian approachs the
estimation of parameters is a highly demanding computacional task, in the
...
The main feature of the parametrization  proposed above is that it enable to use standard matrix algebra to speed up the computation of the posterior distributions without using sampling techniques. This helps to significantly reduce computation time, avoiding recursive methods. In fact, once the posterior distribution of  $u$ is calculated.
...
\end{eqnarray*}
where $\boldsymbol{\mathrm{P}}$ is a diagonal matrix with $\boldsymbol{\mathrm{P}}_{jj} = \frac{(1-u)\lambda_j}{(1-u)\lambda_j+u}$, $j=1,\dots,n$. \\ So, substituting in (\ref{eq:posterior}) is obtained,\\
\begin{eqnarray}\label{eq:posterior1}
\pi(u \,|\, \y) &\propto& u^{\frac{p_0+p}{2}-1}(1-u)^{\frac{n_0+n}{2}-1} |(1-u)\X'\X+u\I|^{-1/2} [(1-u)(SSE(u)+n_0s_0^2)]^{-\frac{n+n_0+p_0}{2}} \nonumber \nonumber \\ && \times\left[1+\frac{u}{1-u}\frac{\hat{\boldsymbol{\beta}}'(u)\hat{\boldsymbol{\beta}}(u)+p_0d_0^2}{SSE(u)+n_0s_0^2}\right]^{-\frac{n+n_0+p_0}{2}} \nonumber \\ &\propto& u^{\frac{p_0+p}{2}-1}(1-u)^{\frac{n_0+n}{2}-1}(1-u)^{-\frac{p}{2}}\begin{vmatrix} \boldsymbol{\Lambda}+\frac{u}{1-u}\I_n &  \boldsymbol{0}_{n,p-n} \nonumber \\ \boldsymbol{0}_{p-n,p} &  \frac{u}{1-u}\I_{p-n}\end{vmatrix}^{1/2} \\ && \times \, [(1-u)(SSE(u)+n_0s_0^2)]^{-\frac{n+n_0+p_0}{2}}\left[1+\frac{u}{1-u}\frac{\hat{\boldsymbol{\beta}}'(u)\hat{\boldsymbol{\beta}}(u)+p_0d_0^2}{SSE(u)+n_0s_0^2}\right]^{-\frac{n+n_0+p_0}{2}} \nonumber \\
...
\end{eqnarray}
Note that the equation (\ref{eq:posterior1}) is used when $n>p$. In other case, $u^{\frac{p+p_0}{2}-1}$ is replaced with $u^{\frac{n+p_0}{2}-1}$. \\
Also note that
\begin{eqnarray*}
...
\end{eqnarray}
For ridge regression the degrees of freedom are commonly calculated by the trace of the matrix that transforms the vector of observations on the dependent variable into the ridge regression estimate of its expected value; that is:
\begin{eqnarray*}
...
In the present work, we can note that it is possible to work high-dimensional ridge regression without the need to use MCMC methods, in addition, the implementation was done in the R software, which allowed us to observe that the computational cost was reduced and with this the time decreased. The results obtained with the implemented package showed that the fit was ``good". In addition, this implementation gives the opportunity to guarantee the convergence of the posterior distribution, moreover, it gives us an idea of what this is like. \\ The small MSE in the cross validation indicates good predictions are obtained.
Despite everything, more studies can be carried out with the data set, it should be remembered that some data may be sensitive to the chosen prior distributions, so it is recommended to carry out a sensitivity study with the given hyperparameters, in this case they were used the default arguments.
